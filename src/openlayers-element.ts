...
export class OpenLayersElement extends LitElement {
  ...
  @state() modeControllers: Array<
    SingleCreate | SingleSelect | TargetRenderer
  > = [];
  ...
  */
  setupTargetBoxSize(geolocationInformation: GeolocationInformation) {
    if (
      geolocationInformation.currentLocation &&
      geolocationInformation.reverseLocation
    )
      this.store.setTargetBoxSize('large');
    else if (
      geolocationInformation.currentLocation ||
      geolocationInformation.reverseLocation
    )
      this.store.setTargetBoxSize('medium');
  ...
  firstUpdated() {
    ...
    this.view = new View({
      ...
      center:
        states.readonly &&
        states.currentSelections &&
        states.currentSelections.length == 1
          ? states.currentSelections[0]
    ...
    if (
      options.search.displaySearch &&
      options.mode.type !== 'target' &&
      !readonly
    ) {
    ...
    if (options.mode.type === 'target') {
      ...
      if (!readonly) {
        ...
        if (options.geolocationInformation.displayBox)
          map.addControl(new TargetInformationBoxElement(this.store));
      }
    }
    if (options.wmts.length > 0) new WMTSLoader(this.store);
    if (options.interaction.displayScaleLine)
      map.addControl(new ScaleLine({ units: 'metric' }));
    if (options.border.url !== '') new Border(this.store, states);
    if (options.inclusionArea.url !== '')
      this.inclusionArea = new InclusionArea(this.store);
    if (options.mode.type === 'select' && options.wfs.url != '') {
      this.modeControllers.push(
        new SingleSelect(this.renderUtils, states, this.store)
      );
    }
      ...
      if (this.states) {
        ...
        if (states.currentSelections.length > 0) {
          switch (this.store.getOptions()?.mode.type) {
            case 'target':
              this.modeControllers[0]?.renderCurrentSelection(states);
              break;
            case 'select':
              this.modeControllers[0]?.renderCurrentSelection(states);
              break;
            ...
            case 'mix':
              this.renderUtils.displayMixMode(
                this.modeControllers[0]?.vectorSource,
                this.modeControllers[1]?.vectorSource,
                states
              );
              break;
          }
          ...
          this.modeControllers.forEach((controller) =>
            controller.removeCurrentSelection()
          );
  ...
  render() {
    return html`
      <div
        id="map"
        class="${this.store.getTargetBoxSize()} ${Store.getTheme()}"
      ></div>
    `;
  }
  ...
  static styles = [
    ...
    unsafeCSS(theme),
    unsafeCSS(animationStyle),
    unsafeCSS(containerStyle),
  ];
}
